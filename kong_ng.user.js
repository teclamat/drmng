...
function main() {
  ...
  window.DRMng = {
    ...
    Gate: {
       ...
       * @param {boolean} [arr = false] request array buffer type
       */
      ...
      lightShotCb: e => {
        ...
        if (i.notNull) {
          ...
          if (img && c)
            i.on('load', () => setTimeout(() => DRMng.PrivateChat.getChat(c).scrollToBottom(), 250))
              .attr({ src: img })
              .remove('id');
          else i.detach();
        }
    ...
    Gestures: {
      Kiss: {
        ...
        getSmittenAdjective: function () {
          return this.smittenAdjective[DRMng.Util.getRand(5)];
        },
        generate: function () {
          let txt = '';
    ...
    Config: {
       ...
       * @returns {*} Data from selected property
       */
      ...
      getRaid(link) {
        ...
        if (match) {
          ...
          if (r) {
            ...
            const ded = DRMng.Util.hasProperty(config.get(`dead::${srv}`), r.id);
    ...
    CSS: class {
      static add(alias, name, value) {
        ...
        const repl = DRMng.Util.hasProperty(DRMng.CSS.rules, alias);
      ...
      static compile(obj) {
        if (DRMng.CSS.node === undefined)
          DRMng.CSS.node = new DomNode('style').attr({ type: 'text/css' }).attach('to', document.head);
    ...
    Kong: {
          ...
          new DomNode('li')
            .data(new DomNode('a')
              ...
              .on('click', e => {
                ...
                new DomNode('#DRMng_KongSlimHeader').txt(isSlim ? 'Full' : 'Slim', true);
                return false;
              }))
      ...
      modifyElement: () => {
        if (Element && Element.Methods) {
          ...
          setTimeout(DRMng.Kong.modifyElement, 10);
        }
      },
      /**
      ...
      modifyChatRoom: () => {
        if (ChatRoom) {
          ...
          setTimeout(DRMng.Kong.modifyChatRoom, 10);
        }
      },
      /**
          ...
          self.addChatCommand(['raid', 'rd'], (chat, cmd) => {
            ...
            if (comm) {
              ...
              if (fnd.length > 1) {
                ...
                chat.serviceMessage('Multiple results found, pick one:' + raidPicker);
              } else if (fnd.length === 1) {
                chat.serviceMessage(DRMng.UI.raidInfo(fnd[0][0]), data[fnd[0][0]].banner);
              } else {
                chat.serviceMessage('No info found matching ' + raid);
              }
            } else {
              chat.serviceMessage('Wrong /raid or /rd syntax');
            }
            return false;
          });
      ...
      hideWorldChat: () => {
        ...
        new DomNode('#game').style({ width: (opts.hideWChat || opts.removeWChat) ? '760px' : '1025px' });
      },
      setWrapperWidth: w => {
    ...
    Raids: {
      ...
      checkAndSend: () => {
        ...
        if (r && !isNaN(+r.id) && r.hash.length === 10 && ['kasan', 'elyssa'][r.pid] === DRMng.Raids.srv) {
          ...
          DRMng.UI.submitResponse(0, 'Paste proper raid link before submitting');
        }
      ...
      getDead: function (id) {
        return id ? DRMng.Util.hasProperty(DRMng.Config.get(`dead::${this.srv}`), id) : false;
      },
    ...
    UM: {
      ...
      getUserNode: function () {
        ...
        if (users.length > 0) {
          ...
          setTimeout(this.getUserNode.bind(this), 100);
        }
      },
      getExtendedUserData: function () {
        if (holodeck && holodeck.ready && holodeck._chat_window._rooms_by_type) {
          if (holodeck._chat_window._rooms_by_type.guild) {
            setTimeout(this.getUserNode.bind(this), 0);
          ...
          setTimeout(this.getExtendedUserData.bind(this), 100);
        }
      },
    ...
    PrivateChat: class {
      ...
      delUser(name) {
        if (!this.userLock) {
          this.userLock = true;
          if (DRMng.Util.hasProperty(this.users.fields, name)) {
      ...
      serviceEvent(data) {
        ...
        switch (data.action) {
          case 'loadData':
            ...
            Object.keys(data.users).forEach(user => this.addUser(data.users[user], true));
        ...
        if (msg) {
          ...
          if (r) {
            let srv = DRMng.Config.local.server.toLowerCase(), g = this.getGuildTag(data.usr.gld),
              ...
              d = DRMng.Util.hasProperty(DRMng.Config.local.dead[srv], r.id),
              l, m = msg[1] + msg[3],
      ...
      static addOption(room) {
        ...
        if (el.notNull) el.node.firstChild.textContent = room.name;
          ...
          .data(
            new DomNode('button').attr({ class: 'l' }).txt('Del').on('click', () => this.removeChat(id)))
    ...
    UI: {
      ...
      displayStatus: function (msg) {
        ...
        if (!msg && (!this.statusTimer || this.statusTimer.timeLeft <= 0)) {
          status.textContent = (DRMng.Raids.joinLen > 0) ?
            DRMng.Raids.count + ' raids, ' + DRMng.Raids.joinLen + ' selected' :
          ...
          if (this.statusTimer) {
            ...
            this.statusTimer = new DRMng.Timer(DRMng.UI.displayStatus.bind(this), 4000);
          }
        }
      },
      ...
      applyFilter: function (e) {
        ...
        if (spanHit) {
          ...
          diff = { n: 1, h: 2, l: 4, nm: 8 }[el.className.split(' ')[0]];
        }
      ...
      setupSidebar: function () {
        ...
        else sb = new DomNode('div').attr({ id: 'DRMng_Sidebar' }).node;
              ...
              if (e.target.files instanceof FileList && e.target.files.length > 0) {
                ...
                reader.addEventListener('load', e => {
                  ...
                  try {
                    ...
                    DRMng.log('error', '{Sidebar::Import} Wrong data format ::', res);
                  }
        ...
        if (boss) {
          ...
          if (r) {
            ...
            txt += Diff.reduce((acc, d) => acc + '<td>' +
              (r.hp[d] ? DRMng.Util.getShortNumK(r.hp[d] * 1000, 4) : '&mdash;') + '</td>', '<tr><td>HP</td>') + '</tr>';
            txt += Diff.reduce((acc, d) => acc + '<td>' +
